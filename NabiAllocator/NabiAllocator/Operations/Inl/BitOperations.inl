...
namespace nabi_allocator::bit_operations
{
	...
	constexpr T FlipBit(T const value, u32 const bitPosition) noexcept
	{
		return value ^ (static_cast<T>(1) << bitPosition);
	}

	template<is_integral T>
	constexpr bool GetBit(T const value, u32 const bitPosition) noexcept
	{
		return static_cast<bool>(value & (static_cast<T>(1) << bitPosition));
	}

	template<is_integral T>
	constexpr T SetBit(T const value, u32 const bitPosition, bool const bitState) noexcept
	{
	...
	template<is_integral T>
	constexpr T LeftShiftBit(T const value, u32 const shiftAmount) noexcept
	{
		return value << shiftAmount;
	}

	template<is_integral T>
	constexpr T RightShiftBit(T const value, u32 const shiftAmount) noexcept
	{
		return value >> shiftAmount;
	}
